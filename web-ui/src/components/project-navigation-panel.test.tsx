...
describe("FeedbackCard", () => {
	...
	// 4. Featurebase loading => disabled, no attribute
	it("shows disabled button without data-featurebase-feedback when pre-identify is loading", () => {
		...
		expect(button!.disabled).toBe(true);
		expect(button!.hasAttribute("data-featurebase-feedback")).toBe(false);
		expect(container.textContent).toContain("Connecting to feedback");
	});

	// 5. Featurebase idle => disabled, no attribute
	it("shows disabled button without data-featurebase-feedback when pre-identify is idle", () => {
	...
	// 6. Featurebase error => disabled + retry
	it("shows disabled button and retry link when pre-identify failed", () => {
		const retryFn = vi.fn();
